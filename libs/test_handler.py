...
class TestFileHandler(unittest.TestCase):
    ...
    def test_read(self):
        """파일 읽기"""
        with patch('builtins.open', mock_open(read_data="test content"), create=True) as mocked_open:
            # when
            result = FileHandler().read(self.TEST_FILENAME)

            # then
            mocked_open.assert_called_once_with(self.TEST_FILENAME, "r", encoding="utf-8")
    ...
    def test_write(self):
        ...
        with patch('builtins.open', mock_open(), create=True) as mocked_open:
            ...
            # then
            mocked_open.assert_called_once_with(self.TEST_FILENAME, 'w', encoding='utf-8')
