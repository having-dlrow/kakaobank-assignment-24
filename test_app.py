...
class TestRegProcessor(unittest.TestCase):
    ...
    def test_정규식분석_디버그모드_활성(self):
        ...
        with patch('libs.debug.Debug.on', True), \
            ...
            # then
            mocked_file.assert_called_once_with(DEBUG_SEARCH_FILE, 'w', encoding='utf-8')
            mocked_findall.assert_called_once_with(REGEX_GREEDY_PATTERN, self.TEST_CONTENT )
...
class TestKiwiProcessor(unittest.TestCase):
    ...
    def test_분석_디버그모드_활성(self, mocked_open):
        ...
        # then
        self.assertEqual(result, expected)
        mocked_open.assert_called_once_with(DEBUG_ANALYZE_FILE, 'w', encoding='utf-8')
    ...
    def test_여러단어_시작글자_명사아님(self):
        ...
        # when
        tokens = [MockToken('고', 'EC'), MockToken('국제', 'NNG'), MockToken('고등학교', 'NNG')]
        result = self.processor._KiwiProcessor__startwith_nnp_and_nouns(tokens)
    ...
    def test_여러단어_명사추출_시작부분연속(self):
        ...
        # when
        tokens = [MockToken('금곡', 'NNP'), MockToken('초등학교', 'NNG'), MockToken('는', 'EC')]
    ...
    def test_여러단어_명사추출_연속안됨(self):
        ...
        # when
        tokens = [MockToken('서울', 'NNP'), MockToken('고', 'EC'),  MockToken('고등학교', 'NNG')]
        match = self.processor._KiwiProcessor__concat_nnp_nng(tokens)
